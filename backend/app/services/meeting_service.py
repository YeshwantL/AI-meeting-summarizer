...
from app.ai.engine import extract_actions, summarize
from app.models.meeting import Meeting
from app.models.task import Task

def create_meeting(db: Session, title: str, transcript: str) -> Meeting:
    meeting=Meeting(title=title, transcript=transcript, summary=summarize(transcript))
    meeting.tasks=[Task(**item) for item in extract_actions(transcript)]
